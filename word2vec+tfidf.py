...
for q in list(data['question1_preprocess']):
    ...
    for word in doc:
        ...
        idf_weight = 0
        if str(word) in tfidfdict: 
            idf_weight = tfidfdict[str(word)]
        mean_words_vec += vec * idf_weight
...
cvscores = cross_val_score(clf1, simvalues.reshape(-1,1),data.is_duplicate, cv=10)
cvscores = [ convert(x) for x in cvscores ]

fd = open('errors.csv','a')
writer = csv.writer(fd, delimiter=";")
for error in cvscores:
  writer.writerow(["word2vec",error])
fd.close()
